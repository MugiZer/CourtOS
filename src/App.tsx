...
import Upcoming, { ScheduleComparison } from './components/Upcoming';
import { DemoPanel, Drawer, HistoryPanel, RulesPanel, SponsorPanel, VoicePanel } from './components/Drawers';
import type { Panel } from './components/Drawers';
import type { CourtId } from './demo/types';
import { getState, markSeen, tickChangeover, unread, useDemo } from './demo/store';
...
export default function App() {
  ...
  const remaining = court.changeoverEndsAt ? Math.max(0, Math.ceil((court.changeoverEndsAt - now) / 1000)) : 0;
  const titles = { rules: ['TOURNAMENT SETTINGS', 'Match rules & scope'], history: ['COURT ' + activeCourt + ' · ' + court.match.id, court.match.phase === 'dispute' ? 'Resolve the dispute.' : 'Score history'], sponsor: ['CHANGEOVER PARTNER', 'Changeover sponsor'], demo: ['LOCAL DEMONSTRATOR', 'Take CourtOS for a spin.'], voice: ['COURT ' + activeCourt, 'Make the call.'] };
  ...
  return <div className={'app ' + (scorer ? 'app--scorer' : '')}>
    ...
    <header className="masthead"><div className="masthead-inner">
      ...
      <div className="masthead-actions"><span className="preview-label" title="Interactive frontend preview with browser-local fixtures"><i/>LOCAL PREVIEW</span><a className="court-link" href={scorer ? '/organizer' : '/court?id=' + activeCourt} target={scorer ? undefined : '_blank'} rel="noreferrer">{scorer ? 'Organizer view' : 'Court display'}<ArrowUpRight size={17}/></a><button className="icon-button inverse" aria-label={muted ? 'Enable and test Time audio' : 'Mute Time audio'} onClick={toggleAudio}>{muted ? <SpeakerSlash size={19}/> : <SpeakerHigh size={19}/>}</button></div>
    </div></header>
    ...
    <main id="main" className="workspace" role="tabpanel" aria-labelledby={tab + '-tab'}>
      ...
      {tab === 'upcoming' ? <Upcoming state={state} onRules={() => openPanel('rules')}/> : <>
        ...
        <DecisionBar court={court} scorer={scorer}/>
        <ScoreControls court={court} scorer={scorer} onVoice={() => openPanel('voice')} onDispute={() => openPanel('history')}/>
        {scorer && <section className="court-audio" aria-label="Time audio setup"><div><strong>TIME at {match.rules.changeover - 10}s · {match.rules.changeover}s rest</strong><span role="status">{audioStatus}</span></div><button className="button" onClick={testAudio}>{muted ? 'Enable & test audio' : 'Test Time call'}</button></section>}
        <CourtVisual court={court} match={match} remaining={remaining} muted={muted} onMute={toggleAudio} scorer={scorer}/></div>
        {state.lastResult && activeCourt === 1 && match.id === 'M103' && <div className="last-result"><Flag size={18}/><strong>M101 complete</strong><span>{state.lastResult}</span><span><Check size={14}/> Result recorded</span></div>}
        <div className="below-court"><Activity court={court} onAll={() => openPanel('history')}/><section className="on-deck"><div className="subsection-heading"><h2>On deck</h2><span className="small-label">{state.plan.status === 'assigned' ? 'PLANNED' : 'READY'}</span></div><div className="on-deck-content"><span className="eyebrow">{state.plan.status === 'assigned' ? 'M104 · CONSOLATION' : 'M103 · SEMIFINAL A'}</span><TeamName names={state.plan.status === 'assigned' ? ['Riley Tran', 'Jordan Moreau'] : ['Noor Ali', 'Émile Gagnon']} compact/><span className="on-deck-vs">{state.plan.status === 'assigned' ? 'Singles · waiting for Court 2' : 'vs Léa Tremblay / Owen Park'}</span><button onClick={() => chooseTab('upcoming')}>See upcoming matches <ArrowRight size={16}/></button></div></section></div>
        {state.plan.status === 'assigned' && activeCourt === 1 && <ScheduleComparison state={state}/>}
        {selectedEvents > 0 && <span className="sr-only" role="status">{selectedEvents} new events on Court {activeCourt}.</span>}
      </>}
    </main>
    <footer className="app-footer"><div><span className="footer-brand">CourtOS</span><span>Every court. Every point. In view.</span></div><div><span className="footer-mode"><Broadcast size={14}/> Browser-local demo</span><button onClick={() => openPanel('demo')}><Play size={13} weight="fill"/> Demo controls <ArrowUpRight size={14}/></button></div></footer>
    {panel && <Drawer key={panel + activeCourt} eyebrow={titles[panel][0]} title={titles[panel][1]} onClose={closePanel} wide={panel === 'history'}>{panel === 'rules' ? <RulesPanel state={state}/> : panel === 'history' ? <HistoryPanel state={state} id={activeCourt}/> : panel === 'sponsor' ? <SponsorPanel state={state}/> : panel === 'voice' ? <VoicePanel state={state} id={activeCourt} onClose={closePanel}/> : <DemoPanel state={state} activeCourt={activeCourt} onScenario={scenario}/>}</Drawer>}
  </div>;
}
